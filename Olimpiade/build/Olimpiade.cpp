...
string Olimpiade::metodo4()
{
    ...
    vector<int> totaleAtleti(nazioni.size(), 0);

    for(int i = 0; i < totaleAtleti.size(); i++){
        for(auto it = atleti.begin(); it != atleti.end(); it++){
            list<string>::iterator pos = nazioni.begin();
            advance(pos, i);
            string corrente = *pos;
            if(it->getNazione() == corrente)
                totaleAtleti[i]++;
        }
    }
    int massimiAtleti = 0;
    int indice = 0;
    for(int i = 0; i < totaleAtleti.size(); i++){
        if(totaleAtleti[i] > massimiAtleti){
            massimiAtleti = totaleAtleti[i];
            indice = i;
        }
    }

    list<string>::iterator nazioneMaggiore = nazioni.begin();
    advance(nazioneMaggiore, indice);

    string atletaVecchio = "";
    int etaMaggiore = 0;
    for(auto it = atleti.begin(); it != atleti.end(); it++){
        if(it->getNazione() == *nazioneMaggiore && it->getEta() > etaMaggiore){
            etaMaggiore = it->getEta();
            atletaVecchio = it->getAtleta();
        }
    }

    return atletaVecchio;
}
